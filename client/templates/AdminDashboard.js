...
export const AdminDashboard = ({ accessLogs = [], users = [] }) => {
  ...
  const [assistanceUserSearch, setAssistanceUserSearch] = useState("");
  ...
  const selectedAssistanceUser = userById.get(assistanceUserId);
  const assistanceUsers = [...assistanceOrganizationUsers, ...(selectedAssistanceUser && !assistanceOrganizationUsers.some((item) => item.id === selectedAssistanceUser.id) ? [selectedAssistanceUser] : [])].filter((item) => item.role === assistanceRole).sort((a, b) => (a.username || "").localeCompare(b.username || "", "it-IT"));
  const assistanceSearchResults = assistanceUserSearch.trim().length < 2 ? [] : users
    .filter((item) => !item.disabled && ["Presidente", "CAS", "GVP"].includes(item.role) && (item.username || "").toLocaleLowerCase("it-IT").includes(assistanceUserSearch.trim().toLocaleLowerCase("it-IT")))
    .sort((a, b) => (a.username || "").localeCompare(b.username || "", "it-IT"))
    .slice(0, 12);
  const presidentIdForAssistanceUser = (account) => {
    ...
    const linkedCasIds = [...new Set([...(account.casIds || []), account.casId, account.associationId].filter(Boolean))];
    const linkedCas = linkedCasIds.map((id) => userById.get(id)).find((item) => item?.role === "CAS");
    return linkedCas?.presidentId || linkedCas?.associationId || "";
  };
  const chooseAssistanceSearchResult = (account) => {
    setAssistancePresidentId(presidentIdForAssistanceUser(account) || "search-result");
    setAssistanceRole(account.role);
    setAssistanceUserId(account.id);
  };
  const openAssistanceModal = () => {
    setAssistancePresidentId("");
    setAssistanceRole("Presidente");
    setAssistanceUserId("");
    setAssistanceUserSearch("");
