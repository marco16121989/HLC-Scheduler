...
export const UsefulFiles = ({ files, currentUser }) => {
  ...
  const canUpload = currentUser.role === "Presidente" || currentUser.role === "CAS";
  ...
  return <>
    ...
    <div className="app-content"><div className="container-fluid"><div className="row g-3">
      ...
      <div className="col-12"><section className="card"><div className="card-header"><h2 className="card-title">Archivio file</h2></div><div className="card-body p-0"><div className="table-responsive"><table className="table table-hover align-middle mb-0"><thead><tr><th>Nome file</th><th>Dimensione</th><th>Caricato da</th><th>Data</th><th className="text-end">Azioni</th></tr></thead><tbody>
        {files.length === 0 ? <tr><td className="text-center text-secondary py-4" colSpan="5">Nessun file caricato.</td></tr> : files.map((file) => {
          const canDelete = currentUser.role === "Presidente" || (currentUser.role === "CAS" && file.createdBy === currentUser.id);
          return <tr key={file.id}><td><button className="btn btn-link fw-medium p-0 text-start" type="button" onClick={() => openFile(file)} title={`Apri ${file.displayName || file.name}`}>{file.displayName || file.name}</button><div className="small text-secondary">{file.name}</div></td><td>{formatSize(file.size)}</td><td>{file.createdByUsername || "-"}</td><td>{file.createdAt ? new Intl.DateTimeFormat("it-IT", { dateStyle: "short", timeStyle: "short" }).format(new Date(file.createdAt)) : "-"}</td><td className="text-end"><div className="d-inline-flex gap-2"><a className="btn btn-outline-primary btn-sm" href={file.dataUrl} download={file.name}>Scarica</a>{canDelete && <button className="btn btn-outline-danger btn-sm" type="button" onClick={() => deleteFile(file)}>Elimina</button>}</div></td></tr>;
        })}
